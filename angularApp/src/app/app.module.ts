...
@NgModule({
  ...
  imports: [
    ...
    JwtModule.forRoot({
      config: {
        tokenGetter,
        whitelistedDomains: ['localhost:5000', `5000${window.location.hostname.substr(4)}`],
        blacklistedRoutes: ['localhost:5000/api/auth', `5000${window.location.hostname.substr(4)}/api/auth`]
      }
    })
  ],
  providers: [
